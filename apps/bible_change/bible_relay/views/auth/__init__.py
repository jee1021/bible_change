...
class RelayCommentInputSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        relay = Relay.objects.filter(verse=verse)
        if not relay :
            raise serializers.ValidationError("해당 구절의 성경릴레이가 존재하지 않습니다.")
        if not relay.is_ongoing():
            raise serializers.ValidationError("현재 진행중인 성경릴레이가 아닙니다.")
        if not relay.check_chapter(chapter):
            raise serializers.ValidationError("현재 진행중인 장이 아닙니다.")
